...
class LookupPropeller(Propeller):
    def __init__(self, data: pd.DataFrame):
        ...
        self._pa_to_t = LinearNDInterpolator((self.data.Pin_d_rho, self.data.airspeed), self.data.thrust_d_rho)
        self._ra_to_t = LinearNDInterpolator((self.data.rpm, self.data.airspeed), self.data.thrust_d_rho)
        self._ra_to_p = LinearNDInterpolator((self.data.rpm, self.data.airspeed), self.data.Pin_d_rho)
        self._ta_to_r = LinearNDInterpolator((self.data.thrust_d_rho, self.data.airspeed), self.data.rpm)
        self._ta_to_p = LinearNDInterpolator((self.data.thrust_d_rho, self.data.airspeed), self.data.Pin_d_rho)
    ...
    def load(diameter: float, prop_name: str):
        return LookupPropeller.from_df(
            diameter, 
            LookupPropeller.proprawdata(prop_name)
        )
